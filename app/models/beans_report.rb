class BeansReport < ActiveRecord::Base
  belongs_to :farmer
  has_one :planting_report, class_name: "BeansReport", foreign_key: "harvest_report_id"
  ...
  def self.search_fields
    return {"created_at" => {type: :time, key: "Report Date", search_column: "beans_reports.created_at"},
  ...
  def self.new_report(session)
    ...
    last_planting_report = BeansReport.where(farmer: farmer, report_type: 'planting').order(created_at: :desc).first
